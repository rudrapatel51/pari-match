...
export const InPlay: React.FC = () => {
  ...
  const allEvents = categories.flatMap(
    (cat: any) => cat.records || cat.events || [],
  );
  const selectedCategory = categories.find(
    (c: any) => (c._id || c.name) === selectedCategoryId,
  );

  const displayEvents = (() => {
    const base = selectedCategory
      ? selectedCategory.records || selectedCategory.events || []
      : allEvents;
  ...
  if (loading && categories.length === 0)
    return <Loader text="Loading live matches..." />;
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex items-center gap-3 mb-4">
        ...
        {totalLive > 0 && (
          <span className="flex items-center gap-1.5 px-3 py-1 bg-accent-green text-brand-text text-xs font-bold rounded-full animate-pulse">
