...
const SpinWinPage: React.FC = () => {
  ...
  const handleSpin = async () => {
    ...
    setSpinResult(null);
    try {
      const res: any = await userApi.playSpin({
        assignment_id: selectedAssignment._id,
      } as any);
      if (res?.success || res?.data) {
        const data = res.data || res;
        const { block_landed, result, won_amount, block_details, message } =
          data;
        const blocks = selectedAssignment.spin_id.blocks;
        const segmentAngle = 360 / blocks.length;
        const targetAnglePositive =
          (360 - (block_landed * segmentAngle + segmentAngle / 2)) % 360;
        const minSpin = 360 * 8;
          ...
          {blocks.map((_: any, i: number) => (
            ...
            >
              <stop
                offset="0%"
                style={{
                  stopColor: blocks[i].block_color || "#1a3a5c",
                  stopOpacity: 1,
                }}
              />
              <stop
                offset="100%"
                style={{
                  stopColor: blocks[i].block_color || "#0d2035",
                  stopOpacity: 0.85,
                }}
              />
            </linearGradient>
            ...
            (loading ? (
              ...
              <div className="bg-bg-card border border-stroke-light rounded-xl shadow-betting-card p-10 text-center">
                ...
                <p className="text-sm text-neutral-gray-600">
                  You don&apos;t have any spins assigned right now.
                </p>
              </div>
            ) : (
              <div
              ...
              >
                ...
                {[
                  ...
                  <span
                    ...
                    style={{
                      animation: `sw-float ${2 + i * 0.4}s ease-in-out infinite`,
                ...
                <div className="relative z-10 p-5 md:p-8">
                  {/* Spin name */}
                  <h2 className="text-center text-xl font-black text-brand-text mb-6 tracking-wide">
                    🚀{" "}
                    <span className="sw-shimmer-text">
                      {selectedAssignment.spin_id?.name?.toUpperCase()}
                    </span>{" "}
                    🚀
                  </h2>
                  ...
                  <div className="relative mx-auto max-w-[400px] aspect-square mb-6">
                    ...
                    <div
                      className="sw-pulse-ring absolute inset-3 rounded-full border border-yellow-400/15"
                    ...
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-2 z-30">
                      <div
                        className={`w-0 h-0 border-l-[18px] border-l-transparent border-r-[18px] border-r-transparent border-t-[36px] border-t-yellow-400 ${spinning ? "" : "sw-pointer-glow"}`}
                      />
                    </div>
                    ...
                    <div className="relative w-full aspect-square">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center z-20">
                        <button
                          ...
                          style={
                            !spinning && !hasSpun
                              ? {
                                  background:
                                    "linear-gradient(135deg, #FF6B00, #FF3D00, #CC0000)",
                                  color: "#fff",
                                }
                              : {}
                          }
                        >
                          {spinning ? (
                            <span className="text-yellow-400 text-xl animate-spin inline-block">
                              ↻
                            </span>
                          ) : hasSpun ? (
                            <span className="text-xs text-neutral-gray-400 font-bold">
                              DONE!
                            </span>
                          ) : (
                            <span className="flex flex-col items-center leading-tight text-brand-text font-black">
                  ...
                  <div className="text-center mb-4">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-yellow-400/10 border border-yellow-400/30 text-yellow-400 text-xs font-semibold">
                      🎯 {mySpins.length} Spin{mySpins.length !== 1 ? "s" : ""}{" "}
                      Available
                    </span>
                  </div>
                  ...
                  {spinResult && (
                    <div className="animate-bounce mt-4">
                      ...
                      >
                        <p className="text-lg font-black text-brand-text mb-1">
                          {spinResult.result === "win"
                            ? "🎉 CONGRATULATIONS! 🎉"
                            : "😔 HARD LUCK!"}
                        </p>
                        <p className="text-sm text-neutral-gray-500 mb-2">
                          {spinResult.message}
                        </p>
                        {(spinResult.block_details?.prize_amount > 0 ||
                          spinResult.won_amount > 0) && (
                          <p className="text-5xl font-black sw-shimmer-text">
                            ₹
                            {spinResult.block_details?.prize_amount ||
                              spinResult.won_amount}
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <thead>
                      <tr className="border-b border-stroke-light bg-bg-light-blue">
                        {["Date", "Game", "Status", "Prize"].map((h) => (
                          <th
                            key={h}
                            className="px-4 py-3 text-left text-xs font-semibold text-brand-text uppercase tracking-wider whitespace-nowrap"
                          >
                            {h}
                          </th>
                        ))}
                    ...
                    <tbody className="divide-y divide-stroke-light">
                      {history.assignments?.map((item: any) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <span
                              className={`px-2 py-0.5 rounded-full text-xs font-semibold ${item.status === "played" ? "bg-accent-green/10 text-accent-green" : "bg-accent-yellow/10 text-accent-yellow"}`}
                            >
                              {item.status}
                            </span>
                          </td>
                          <td
                            className={`px-4 py-3 font-mono font-bold ${item.won_amount > 0 ? "text-accent-green" : "text-neutral-gray-600"}`}
                          >
                      ...
                      {(!history.assignments ||
                        ...
                        <tr>
                          <td
                            colSpan={4}
                            className="px-4 py-10 text-center text-sm text-neutral-gray-600"
                          >
