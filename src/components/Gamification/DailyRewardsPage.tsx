...
const DailyRewardsPage: React.FC = () => {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      if (!claimed_today && d.last_claim_date) {
        ...
        const lastDate = new Date(d.last_claim_date)
          .toISOString()
          .split("T")[0];
      ...
      setStatus({
        ...
        daily_rewards:
          Array.isArray(d.daily_rewards) && d.daily_rewards.length
            ? d.daily_rewards
            : undefined,
      });
    } catch {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      toast.error(
        err?.message || "Already claimed today or check-in unavailable",
      );
    } finally {
      setClaiming(false);
    }
    ...
    <div className="min-h-screen bg-bg-primary">
      ...
      <div className="bg-brand-primary-dark px-6 py-5">
        ...
        <p className="text-sm text-brand-text-70 mt-0.5">
          Check in every day to earn bonus coins
        </p>
      </div>

      <div className="p-4 md:p-6 max-w-2xl space-y-5">
        {/* Streak summary card */}
        <div className="bg-bg-card border border-stroke-light shadow-betting-card p-5 flex items-center gap-4">
          <div className="w-12 h-12 bg-brand-text-10 flex items-center justify-center flex-shrink-0">
            <FiStar className="w-6 h-6 text-brand-text-70" />
          </div>
          <div>
          ...
          <div className="p-5">
            <div className="grid grid-cols-7 gap-3">
              {rewards.map((reward, i) => {
                ...
                const isToday =
                  day === status.streak + 1 && !status.claimed_today;
                const isTodayClaimed =
                  day === status.streak && status.claimed_today;
                      ...
                      isTodayClaimed
                        ...
                        : claimed
                          ...
                          : isToday
                            ...
                            : "bg-brand-text-10 border-stroke-light opacity-60"
                    }`}
                  >
                    <p className="text-[10px] text-brand-text-70 mb-0.5">
                      Day {day}
                    </p>
                    <p
                    ...
                    <div className="mt-1.5 flex justify-center">
                      {claimed || isTodayClaimed ? (
                        ...
                        <FiLock className="w-3 h-3 text-brand-text-50" />
                      ) : null}
                    </div>
                  </div>
                );
        ...
        {/* CTA card */}
        <div className="bg-bg-card border border-stroke-light shadow-betting-card p-5 text-center">
          {status.claimed_today ? (
            <div className="inline-flex items-center gap-2 px-6 py-3 bg-accent-green/10 border border-accent-green text-accent-green font-semibold text-sm">
            ...
            <button
              ...
              disabled={claiming}
              className="px-8 py-3 bg-accent-green text-black font-bold text-base disabled:opacity-50 hover:opacity-90 transition-opacity"
            >
              {claiming
                ? "Claiming…"
