...
type CategoryFilter = 'all' | 'general' | 'system' | 'user' | 'admin' | 'security' | 'promotion';
...
const NotificationPage: React.FC = () => {
    const {
        ...
        markAsRead, markAllAsRead, isLoading, setLoading,
    ...
    const handleCategoryChange = (val: CategoryFilter) => { setCategoryFilter(val); setPage(1); };
    ...
    const handleMarkAll = async () => {
        ...
        try {
            await Promise.all(
                unread.map((n) => userApi.changeNotificationStatus({ id: n._id, status: 1 }))
            );
            markAllAsRead();
        } catch {}
                                ...
                                className={`
                                    ...
                                    ${statusFilter === opt.value
                                        /*
                                          ...
                                          nav color, text-white always reads on it.
                                        */
                        ...
                        {notifications.map((n) => (
                            ...
                            >
                                <div className="flex items-start gap-3">
                                    ...
                                    {/* Icon */}
                                    <div className={`p-2 rounded-lg flex-shrink-0 mt-0.5 ${getIconBg(n)}`}>
