...
const EventCardGrid: React.FC<EventCardGridProps> = ({ events, groupByLeague = true, loading = false }) => {
                    ...
                    {groupByLeague && (
                        <div className="mb-4">
                            <div className="flex items-center gap-3 pb-2 border-b border-stroke-light">
                                ...
                                <h3 className="text-sm font-bold text-white uppercase tracking-wide">{league}</h3>
                            </div>
                        </div>
                    )}
                                ...
                                >
                                    ...
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-2 mt-0.5">
                                                {timeStr && (
                                                    <p className="text-xs text-neutral-gray-400">{timeStr}</p>
                                                )}
                                                {e.hasBookmaker && (
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 min-w-0 flex flex-col gap-1.5">
                                            {runners.length >= 1 && (
                                                <>
                                                    {runners.slice(0, Math.min(runners.length, 2)).map((runner, idx) => (
                                                        <div key={idx} className="text-xs text-neutral-gray-300 truncate">
                                                            {runner.name}
                                                        </div>
                                                    ))}
                                                </>
                                        ...
                                        {runners.length > 0 && (
                                            <div className={`flex gap-1.5 flex-shrink-0`}>
                                                {runners.slice(0, Math.min(runners.length, 2)).map((runner, runnerIdx) => (
                                                    <div key={runnerIdx} className="flex gap-1.5 flex-col">
                                                        {/* Runner name above odds */}
                                                        <div className="text-xs text-neutral-400 text-center truncate px-1">
                                                            {runner.name.length > 12 ? runner.name.slice(0, 10) + '...' : runner.name}
                                                        </div>
                                                        ...
                                                        <div className="flex gap-1.5">
                                                            ...
                                                            >
                                                                <span className={`font-bold text-cyan-400 group-hover:text-white leading-none transition-colors ${
                                                                    runners.length === 2
                                                                        ? 'text-xl sm:text-2xl'
                                                                        : runners.length === 3
                                                                        ? 'text-lg sm:text-xl'
                                                            ...
                                                            >
                                                                <span className={`font-bold text-pink-300 group-hover:text-white leading-none transition-colors ${
                                                                    runners.length === 2
                                                                        ? 'text-xl sm:text-2xl'
                                                                        : runners.length === 3
                                                                        ? 'text-lg sm:text-xl'
