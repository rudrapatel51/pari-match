...
export function parseScorecardHTML(html?: string): [string, string] {
    if (!html) return ['', ''];
    try {
        const runs = [...html.matchAll(/<span[^>]*class="run"[^>]*>([^<]+)<\/span>\s*<span[^>]*class="over"[^>]*>([^<]+)<\/span>/gi)];
        if (runs.length >= 2) return [`${runs[0][1].trim()} ${runs[0][2].trim()}`, `${runs[1][1].trim()} ${runs[1][2].trim()}`];
        if (runs.length === 1) return [`${runs[0][1].trim()} ${runs[0][2].trim()}`, ''];
    } catch (_) { /* ignore */ }
    return ['', ''];
}
...
export function getScores(event: SportEvent): [string, string] {
    if (event.score && event.score.trim()) {
        ...
        if (idx !== -1) return [event.score.slice(0, idx).trim(), event.score.slice(idx + 3).trim()];
...
export const EventCard: React.FC<{ event: SportEvent }> = ({ event }) => {
    ...
    const isLive    = !!(event.isInPlay || event.matchStatus === 'LIVE' || event.matchStatus === 'in_play');
    ...
    const teams = [{ name: team1, score: score1 }, { name: team2, score: score2 }].filter(t => t.name);
    const runners = event.matchOdds?.runners ?? [];
    ...
    return (
        <div
            className="flex-shrink-0 w-full md:w-[calc(33.333%-8px)] bg-bg-card border border-stroke-light rounded-2xl shadow-betting-card
        ...
        >
            ...
            <div className="flex items-center justify-between px-3 py-2.5 border-b border-stroke-light">
                <div className="flex items-center gap-1.5 overflow-hidden">
                    {isLive ? (
                        <span className="w-2 h-2 rounded-full bg-accent-green animate-pulse flex-shrink-0" />
                    ) : (
                        <Pencil className="w-3 h-3 text-brand-primary flex-shrink-0" strokeWidth={2.2} />
                    )}
            ...
            {/* Teams + scores + odds */}
            <div className="px-3 pb-3 flex flex-col gap-0 flex-1">
                {/* Column headers for odds */}
                {runners.length > 0 && (
                    <div className="flex items-center justify-end gap-1 mb-1">
                        <span className="text-[10px] font-semibold text-neutral-gray-500 min-w-[40px] text-center">Back</span>
                        <span className="text-[10px] font-semibold text-neutral-gray-500 min-w-[40px] text-center">Lay</span>
                    </div>
                )}

                {teams.map((team, i) => {
                    const runner = runners[i];
                    const back = runner?.back != null && runner.back !== '' ? Number(runner.back) : null;
                    const lay  = runner?.lay  != null && runner.lay  !== '' ? Number(runner.lay)  : null;
                    return (
                        <div key={i} className="flex items-center justify-between gap-2 py-1 border-b border-stroke-light last:border-0">
                            {/* Team name + score */}
                            ...
                            {runner && (
                                <div className="flex items-center gap-1 flex-shrink-0">
                                    <span className="min-w-[40px] text-center text-[12px] font-semibold bg-odds-back text-neutral-gray-900 rounded px-1 py-0.5 tabular-nums">
                                        {back != null && !isNaN(back) ? back.toFixed(2) : '—'}
                                    </span>
                                    <span className="min-w-[40px] text-center text-[12px] font-semibold bg-odds-lay text-neutral-gray-900 rounded px-1 py-0.5 tabular-nums">
            ...
            {/* Footer */}
            <button
                onClick={e => { e.stopPropagation(); goToEvent(); }}
                className="w-full py-2.5 text-[12px] font-medium text-neutral-gray-500
                           border-t border-stroke-light bg-bg-card
