    ...
    return (
        ...
        >
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0">
                {isLive ? (
                    <span className="bg-accent-green text-white text-[9px] font-bold px-1.5 py-0.5 rounded uppercase leading-none tracking-wide flex-shrink-0 animate-pulse">
                        LIVE
                    </span>
                ) : (
                ...
                <div className="min-w-0 flex-1">
                    {away ? (
                        <div className="flex flex-col gap-0.5 min-w-0">
                            <span className="text-xs font-semibold text-brand-text truncate">{home}</span>
                            <span className="text-xs font-semibold text-brand-text truncate">{away}</span>
                        </div>
                    ) : (
                        <span className="text-xs font-semibold text-brand-text truncate block">{event.name}</span>
                    )}
                    {event.hasBookmaker && (
                        <span className="text-[9px] text-accent-yellow font-medium leading-none">BM</span>
                    )}
                </div>
            </div>
            ...
            <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                {hasOdds ? (
                    runners.slice(0, 2).map(r => (
                        <div key={r.runnerId} className="flex flex-col items-center gap-0.5">
                    ...
                    <span className="text-[10px] text-neutral-gray-400 italic">No odds</span>
                )}
            </div>
        </div>
    );
