...
import {
    FiSmartphone, FiUser, FiCode, FiCheck,
    FiEye, FiEyeOff,
} from 'react-icons/fi';
...
const RegisterForm: React.FC<RegisterFormProps> = ({ onSuccess }) => {
    ...
    const {
        registerSendOtp, isSendingRegisterOtp,
        registerVerifyOtp, isRegistering,
        countryCodes, isLoadingCountryCodes,
    } = useAuth();
    ...
    useEffect(() => {
        ...
        const interval = window.setInterval(() => setOtpTimer(prev => prev - 1), 1000);
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex items-center gap-3">
                <div className={`flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold shrink-0 transition-colors ${step === 'details'
                    ? 'bg-brand-primary text-white'
                    : 'bg-brand-accent text-black'
                    }`}>
                    {step === 'details' ? '1' : <FiCheck className="w-3.5 h-3.5" />}
                </div>
                <div className={`flex-1 h-0.5 transition-colors ${step === 'otp' ? 'bg-brand-primary' : 'bg-stroke-light'}`} />
                <div className={`flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold shrink-0 transition-colors ${step === 'otp'
                    ? 'bg-brand-primary text-white'
            ...
            <div className="flex justify-between text-xs px-0 -mt-2">
                <span className={step === 'details' ? 'text-brand-text font-semibold' : 'text-accent-green font-semibold'}>
                    Your Details
                </span>
                <span className={step === 'otp' ? 'text-brand-text font-semibold' : 'text-neutral-gray-600'}>
                    Verify OTP
                </span>
            </div>
            ...
            >
                ...
                {step === 'details' && (
                    <>
                        ...
                        <div className="flex gap-2">
                            ...
                            >
                                {countryCodes.map((cc) => (
                                    <option key={cc._id} value={cc.number}>{cc.number}</option>
                                ))}
                            </select>
                        ...
                        <div className="relative">
                            ...
                            >
                                {showPassword ? <FiEyeOff className="w-4 h-4 text-brand-text" /> : <FiEye className="w-4 h-4 text-brand-text" />}
                            </button>
                        </div>
                        ...
                        <label className="flex items-start gap-2.5 cursor-pointer group">
                            ...
                            <span className="text-sm text-brand-text transition-colors leading-snug">
                                ...
                                <a href="/terms" className="text-brand-text underline font-medium">
                ...
                {step === 'otp' && (
                    <div className="space-y-4">
                        <div className="text-center space-y-0.5">
                            ...
                            <p className="font-semibold text-brand-text">{countryCode} {mobile}</p>
                        </div>
                ...
                >
                    {step === 'details'
                        ? (isSendingRegisterOtp ? 'SENDING OTP...' : 'SEND OTP & CONTINUE')
                        : (isRegistering ? 'CREATING ACCOUNT...' : 'VERIFY & CREATE ACCOUNT')
                    }
                </button>
