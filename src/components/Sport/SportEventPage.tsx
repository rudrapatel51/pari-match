...
const SportHeroBanner: React.FC<{ sportName: string; sportIcon: string }> = ({ sportName, sportIcon }) => {
    ...
    if (loading) return <div className="h-36 sm:h-44 md:h-52 bg-brand-primary-dark animate-pulse w-full" />;
            ...
            <div className="relative h-36 sm:h-44 md:h-52 overflow-hidden select-none bg-gradient-to-r from-brand-primary-dark to-brand-primary">
                <div className="relative z-10 flex flex-col justify-center h-full px-6 sm:px-10">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h2 className="text-white font-extrabold text-2xl sm:text-3xl uppercase drop-shadow">{sportName}</h2>
                    </div>
                    <p className="text-white/80 text-sm max-w-sm">Live odds and real-time markets. Bet now!</p>
    ...
    return (
        <div className="relative h-36 sm:h-44 md:h-52 overflow-hidden select-none">
            {imgSrc ? (
                <img src={imgSrc} alt={banner.title || sportName} className="w-full h-full object-cover" />
            ) : (
...
const SportEventPage: React.FC<SportEventPageProps> = ({ sportId, sportName, sportIcon }) => {
    ...
    // Group by league
    const leagueGroups = searchedEvents.reduce<Record<string, SportEvent[]>>((acc, e) => {
    ...
    return (
        <div className="w-full max-w-[1920px] mx-auto">
            ...
            {/* 3. TOP EVENTS */}
            <SportsEventsSection externalEvents={topEvents as any} totalCount={topEvents.length} />
        ...
        CLAIM NOW
            ...
            {leagues.length > 0 && (
                <div className="px-3 sm:px-4 pt-3 pb-4">
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <a href="#all-events" className="text-white/80 text-xs font-semibold hover:text-white underline">All leagues</a>
                    </div>
            ...
            <div id="all-events" className="bg-bg-light-blue">
                <div className="bg-brand-primary px-3 sm:px-4 py-2.5 flex items-center gap-2">
                    ...
                    <h2 className="text-white font-bold text-sm uppercase tracking-wide">{sportName} — All Events</h2>
                    <span className="text-white/60 text-xs ml-auto">{events.length} events</span>
                </div>
                    ...
                    <div className="flex items-center overflow-x-auto scrollbar-hide flex-1">
                        {filterTabs.map(tab => (
                            <button
                                ...
                                className={`flex items-center gap-1 px-3 sm:px-4 py-2.5 text-xs font-semibold whitespace-nowrap border-r border-stroke-light transition-colors flex-shrink-0 ${activeFilter === tab
                ...
                <div className="pb-4">
                    {loading ? (
                        <div className="px-3 pt-3 space-y-2">
                            {[...Array(6)].map((_, i) => (
                                <div key={i} className="h-14 bg-stroke-light animate-pulse rounded" />
                            ))}
                        </div>
                    ) : searchedEvents.length === 0 ? (
