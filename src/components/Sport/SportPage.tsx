...
export const SportPage: React.FC<SportPageProps> = ({ slug, title }) => {
  ...
  const seriesGroups = events.reduce(
    (acc: Record<string, Event[]>, event: any) => {
      const key =
    ...
    <div className="w-full max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold font-display text-brand-text capitalize">
            {title}
          </h1>
          ...
          {liveCount > 0 && (
            <span className="flex items-center gap-1 px-2 py-0.5 bg-accent-green text-brand-text text-xs font-bold rounded animate-pulse">
              LIVE {liveCount}
            </span>
          )}
        </div>
              ...
              seriesKeys.map((seriesKey) => (
                <div key={seriesKey}>
                  <div className="px-4 py-2 bg-bg-light-blue border-b border-stroke-light">
                    <h3 className="text-xs font-bold text-neutral-gray-400 uppercase tracking-wide">
                      {seriesKey}
                    </h3>
                  </div>
