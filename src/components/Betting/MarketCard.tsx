...
const SESSION_TYPES = [
    ...
    'meter', 'player-race', 'odd-even', 'other-market',
];
 ...
 */
function buildRunnerOddsMap(oddsData: any, market: Market): Record<string, any> {
...
function buildFallbackOddsMap(market: Market): Record<string, any> {
    ...
    for (const runner of market.runners) {
        if (runner.metadata && (runner.metadata.b1 !== undefined || runner.metadata.l1 !== undefined)) {
...
function getSuspensionOverlay(runnerOdds: any, marketData: any): string | null {
    ...
    if (marketData?.s === false || marketData?.s === 'SUSPENDED') return 'SUSPENDED';
    return null;
}
...
function MarketHeader({
    ...
    return (
        <div className="flex items-center justify-between px-3 py-2 text-white rounded-t bg-brand-primary">
            {/* Left: market name + cashout badge */}
            <div className="flex items-center gap-2 flex-1 min-w-0 overflow-hidden">
                <span className="font-display text-sm font-semibold truncate">{market.name}</span>
            ...
            <div className="flex items-center gap-2 shrink-0">
                ...
                <button className="text-white/60 hover:text-white transition-colors p-0.5" title="Market Info">
                    <FiInfo size={13} />
                </button>
                <button
...
export default function MarketCard({
    ...
    const isRunnerInSlip = (runnerId: string, betType: 'BACK' | 'LAY') =>
        betItems.some(
            (i) => i.marketId === market.marketId && i.runnerId === runnerId && i.betType === betType
        );
    ...
    const getRunnerPnl = (runnerId: string): string => {
        ...
        const isSession = SESSION_PNL_TYPES.includes((item.marketType || '').toLowerCase());
        const pnl = isSession
            ? item.betType === 'BACK' ? (item.odds * item.stake) / 100 : -((item.odds * item.stake) / 100)
            : item.betType === 'BACK' ? (item.odds - 1) * item.stake : -((item.odds - 1) * item.stake);
    ...
    if (SESSION_TYPES.includes(type)) {
        return (
            <SessionMarketRow
                ...
                useNoYes={['session', 'fancy', 'fancy1', 'ball-by-ball', 'khado', 'meter', 'player-race'].includes(type)}
            />
        );
    }
                        ...
                        <div key={runner.runnerId} className="flex items-center px-2 py-2 gap-1.5">
                            ...
                            <div className="relative flex gap-0.5 shrink-0">
                                {overlay ? (
                                    ...
                                    <>
                                        <OddsButton
                                            odds={ro.b3} volume={ro.br3 || 0} type="back" size="sm"
                                            active={isRunnerInSlip(runner.runnerId, 'BACK')}
                                            onClick={() => handleOddsClick(runner, 'BACK', ro.b3)}
                                        />
                                        <OddsButton
                                            odds={ro.b2} volume={ro.br2 || 0} type="back" size="sm"
                                            active={isRunnerInSlip(runner.runnerId, 'BACK')}
                                            onClick={() => handleOddsClick(runner, 'BACK', ro.b2)}
                                        />
                                        <OddsButton
                                            odds={ro.b1 ?? ro.b} volume={ro.br1 || ro.br || 0} type="back" size="md"
                                            active={isRunnerInSlip(runner.runnerId, 'BACK')}
                                            onClick={() => handleOddsClick(runner, 'BACK', ro.b1 ?? ro.b)}
                                        />
                                        <OddsButton
                                            odds={ro.l1 ?? ro.l} volume={ro.lr1 || ro.lr || 0} type="lay" size="md"
                                            active={isRunnerInSlip(runner.runnerId, 'LAY')}
                                            onClick={() => handleOddsClick(runner, 'LAY', ro.l1 ?? ro.l)}
                                        />
                                        <OddsButton
                                            odds={ro.l2} volume={ro.lr2 || 0} type="lay" size="sm"
                                            active={isRunnerInSlip(runner.runnerId, 'LAY')}
                                            onClick={() => handleOddsClick(runner, 'LAY', ro.l2)}
                                        />
                                        <OddsButton
                                            odds={ro.l3} volume={ro.lr3 || 0} type="lay" size="sm"
                                            active={isRunnerInSlip(runner.runnerId, 'LAY')}
                                            onClick={() => handleOddsClick(runner, 'LAY', ro.l3)}
                                        />
                                    </>
...
function SessionMarketRow({
    market, oddsData, handleOddsClick, isRunnerInSlip, useNoYes,
}: {
    market: Market;
    oddsData: any;
    handleOddsClick: (runner: any, betType: 'BACK' | 'LAY', odds: any, line?: number | null) => void;
    ...
    const rawBr = oddsData?.br ?? oddsData?.bs  ?? oddsData?.bs1 ?? market.runners?.[0]?.metadata?.bs1 ?? 0;
    const rawLr = oddsData?.lr ?? oddsData?.ls  ?? oddsData?.ls1 ?? market.runners?.[0]?.metadata?.ls1 ?? 0;
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-stroke-light bg-bg-card hover:bg-bg-light-blue transition-colors">
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0">
                {market.ballNumber ? (
                    ...
                    <span className="text-neutral-gray-300 shrink-0 text-base leading-none">♡</span>
                )}
                <span className="text-sm font-medium text-brand-text truncate">{market.name}</span>
                <button className="shrink-0 text-[10px] font-bold border border-stroke-primary text-white px-1.5 py-0.5 rounded hover:bg-brand-primary-light hover:border-brand-primary-light transition-colors">
                    BOOK
                </button>
            </div>
            ...
            <div className="relative flex gap-1 shrink-0">
                ...
                <button className="text-neutral-gray-600 hover:text-brand-primary dark:hover:text-brand-text transition-colors p-1" title="Market Info">
                    <FiInfo size={12} />
                </button>
            </div>
        </div>
