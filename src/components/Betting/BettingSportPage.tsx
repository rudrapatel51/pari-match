...
export default function BettingSportPage() {
    ...
    useEffect(() => {
        ...
        bettingClient
            ...
            .catch(() => { useToastStore.getState().error('Failed to load sport events'); });
        ...
        <div className="p-3">
            {events.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 gap-3">
                    ...
                    <p className="text-neutral-gray-700 text-sm">No events found for this sport</p>
                </div>
            ) : (
                        ...
                        >
                            <div className="flex-1 flex flex-col gap-1 min-w-0">
                                <div className="flex items-center gap-2">
                                    <span className="text-xs text-neutral-gray-600 shrink-0">{formatDate(event.startTime)}</span>
                                    <span className="text-xs text-neutral-gray-600 shrink-0">{formatTime(event.startTime)}</span>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {event.hasBookmaker && (
                                        <>
                                            <span className="text-[10px] bg-brand-primary text-white font-bold px-1.5 py-0.5 rounded shrink-0">BM</span>
                                            <FiTv size={12} className="text-neutral-gray-600 shrink-0" />
                                        </>
                                    )}
                                </div>
                            </div>
