...
export default function MatchedBetsPanel({ eventId }: Props) {
        ...
        async (perpage = 10) => {
            ...
            try {
                ...
                if (data?.status === 200 || data?.records) {
                    const recs: MatchBetRecord[] = data?.data?.records ?? data?.records ?? [];
                    const tot: number = data?.data?.pagiantion?.total ?? data?.pagiantion?.total ?? recs.length;
                    setRecords(recs);
                    setTotal(tot);
    ...
    return (
        <div className="mx-2 mt-2 mb-0 rounded-lg overflow-hidden shadow-betting-card border border-stroke-light">
            ...
            <button
                onClick={() => setOpen((v) => !v)}
                className="w-full flex items-center justify-between px-3 py-2.5 bg-brand-primary text-white hover:bg-brand-primary-light transition-colors"
            >
                <div className="flex items-center gap-2">
                    <FiList size={14} className="shrink-0" />
                    <span className="font-display text-sm font-semibold">My Matched Bets</span>
                    {records.length > 0 && (
                        <span className="text-[10px] bg-white/20 text-white font-bold px-1.5 py-0.5 rounded-full leading-none">
                            {records.length}
                        </span>
                    )}
                </div>
                {open
                    ? <FiChevronUp  size={16} className="shrink-0 text-white/80" />
                    : <FiChevronDown size={16} className="shrink-0 text-white/80" />
                }
            </button>
            ...
            {open && (
                <div className="bg-bg-card">
                    ...
                    <div className="grid grid-cols-[1fr_52px_52px_68px] px-3 py-2 border-b border-stroke-light bg-brand-primary-dark">
                        ...
                        <span className="text-[10px] font-bold text-white/60 uppercase tracking-wide text-center">
                            Odds
                        </span>
                        <span className="text-[10px] font-bold text-white/60 uppercase tracking-wide text-center">
                            Stake
                        </span>
                        <span className="text-[10px] font-bold text-white/60 uppercase tracking-wide text-right">
                            P&amp;L
                        </span>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <>
                            {records.map((bet) => {
                                ...
                                const isBack   = bet.bet_type === 'BACK' || bet.bet_type === 'YES';
