...
const STATUS_COLORS: Record<string, string> = {
    MATCHED: 'text-white bg-bg-light-blue',
...
export default function BettingMyBetsPage() {
    ...
    const { activeBets, settledBets, setActiveBets, setSettledBets } = useBettingStore();
    ...
    useEffect(() => {
        ...
        req
            .then((res: any) => {
                const bets: PlacedBet[] = res?.data?.bets || res?.bets || res?.data || res || [];
    ...
    return (
        <div className="p-3">
            {/* Page title */}
            <div className="mb-4">
                <h1 className="font-display text-xl font-bold text-neutral-gray-800">My Bets</h1>
            </div>
                ...
                <button
                    ...
                    className={clsx(
                        ...
                        filter === 'active'
                            ...
                            : 'text-neutral-gray-700 hover:text-white'
                    )}
                >
                    <FiClock size={14} />
                    Active
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
                    ...
                    <p className="text-neutral-gray-700 text-sm">No {filter} bets found</p>
                </div>
            ) : (
                <div className="space-y-3">
                    {bets.map((bet, idx) => {
                        ...
                        const potentialReturn = bet.betType === 'BACK'
                            ? isSession ? (bet.odds * bet.stake) / 100 + bet.stake : (bet.odds - 1) * bet.stake + bet.stake
                            : bet.stake;
                        const liability = bet.betType === 'LAY'
                            ? isSession ? (bet.odds * bet.stake) / 100 : (bet.odds - 1) * bet.stake
                            : bet.stake;

                        return (
                            <div key={(bet as any).betId || idx} className="bg-bg-card rounded-lg shadow-betting-card overflow-hidden">
                                {/* Card header */}
                                <div className="flex items-center justify-between px-3 py-2 bg-bg-light-blue border-b border-stroke-light">
                                    <div className="flex items-center gap-2">
                                        <span className={clsx(
                                            'text-xs font-bold px-2 py-0.5 rounded uppercase',
                                            bet.betType === 'BACK' ? 'bg-odds-back text-neutral-gray-900' : 'bg-odds-lay text-neutral-gray-900'
                                        )}>
                                            {bet.betType}
                                        </span>
                                    ...
                                    {bet.status && (
                                        <span className={clsx(
                                            'text-xs font-bold px-2 py-0.5 rounded uppercase',
                                            STATUS_COLORS[bet.status] ?? 'text-neutral-gray-700 bg-bg-light-blue'
                                        )}>
                                            {bet.status}
                                        </span>
                                    )}
                                ...
                                <div className="px-3 py-3">
                                    <p className="text-sm font-semibold text-neutral-gray-800 mb-2">{bet.runnerName}</p>
                                    <div className="grid grid-cols-3 gap-2 text-xs">
                                        <div>
                                            <span className="text-neutral-gray-600 block">Odds</span>
                                            <span className="font-mono font-bold text-neutral-gray-800">{Number(bet.odds).toFixed(2)}</span>
                                        </div>
                                        <div>
                                            <span className="text-neutral-gray-600 block">Stake</span>
                                            <span className="font-mono font-bold text-neutral-gray-800">₹{Number(bet.stake).toFixed(2)}</span>
                                        </div>
                                        <div>
                                            ...
                                            <span className={clsx(
                                                'font-mono font-bold',
                                                bet.betType === 'LAY' ? 'text-accent-red' : 'text-accent-green'
                                            )}>
                                                ₹{(bet.betType === 'BACK' ? potentialReturn : liability).toFixed(2)}
                                            </span>
                                        </div>
                                    </div>
                                </div>
