...
const InPlayPanel: React.FC = () => {
    ...
    const selectedCat = categories.find((c: any) => (c._id || c.name) === selectedCatId);
    const displayEvents = selectedCat ? (selectedCat.records || selectedCat.events || []) : allEvents;
        ...
        <div>
            ...
            {allEvents.length > 0 && (
                <div className="px-4 pt-3 pb-1">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-accent-green text-white text-xs font-bold rounded-full animate-pulse">
...
const SportPanel: React.FC<SportPanelProps> = ({ tabId, title }) => {
    ...
    // Group by series
    const seriesGroups = events.reduce((acc: Record<string, any[]>, event: any) => {
    ...
    return (
        <div>
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b border-stroke-light">
                <span className="text-xs text-brand-text font-semibold">{events.length} matches</span>
                {liveCount > 0 && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-accent-green text-white text-xs font-bold rounded animate-pulse">
                        LIVE {liveCount}
                    </span>
                )}
            </div>
    ...
    return (
        <div className="min-h-screen bg-bg-primary">
            ...
            <div className="bg-brand-primary px-6 py-5">
                ...
                <p className="text-sm text-white/60 mt-0.5">Sports betting — live &amp; upcoming</p>
            </div>
