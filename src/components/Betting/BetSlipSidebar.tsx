  ...
  const handlePlaceBets = async () => {
    ...
    try {
      ...
      useToastStore
        .getState()
        .success(
          `${betItems.length} bet${betItems.length > 1 ? "s" : ""} placed successfully`,
        );
    } catch (err: any) {
    ...
    <>
      ...
      {betItems.length > 0 && !isBetSlipOpen && (
        <button
          ...
          className="fixed bottom-20 right-4 z-50 bg-brand-primary text-brand-text rounded-full px-4 py-2.5 shadow-elevated font-display text-sm font-bold flex items-center gap-2 hover:bg-brand-primary-light transition-colors"
        >
      ...
      >
        ...
        {/* ── Bet Slip Header ── */}
        <div className="flex items-center justify-between px-4 py-3 text-brand-text shrink-0 bg-brand-primary">
          ...
          <div className="flex items-center gap-2">
            {betItems.length > 0 && (
              <button
                ...
                className="text-brand-text/70 hover:text-brand-text text-xs flex items-center gap-1 transition-colors"
              >
          ...
          {lastResult && (
            <div className="flex flex-col items-center justify-center h-full gap-4 p-6 text-center">
              <div className="w-16 h-16 bg-accent-green rounded-full flex items-center justify-center">
                <span className="text-brand-text text-2xl">✓</span>
              </div>
              <h3 className="font-display text-lg font-bold text-neutral-gray-900">
                Bets Placed!
              </h3>
              ...
              <button
                onClick={() => {
                  useBettingStore.getState().setLastResult(null);
                  toggleBetSlip();
                }}
              ...
              {betItems.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={clsx(
                      "bg-bg-light-blue rounded-lg p-3 border",
                      item.oddsChanged
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1 min-w-0">
                        <span
                          className={clsx(
                            "text-xs font-bold px-2 py-0.5 rounded uppercase mr-2",
                            item.betType === "BACK"
                      ...
                      <button
                        ...
                        className="text-neutral-gray-600 hover:text-accent-red transition-colors ml-2 shrink-0"
                      >
                        <FiX size={14} />
                      </button>
                    </div>
                    <p className="text-xs text-neutral-gray-700 mb-2 truncate">
                      {item.marketName}
                    </p>
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-neutral-gray-700 text-sm font-mono">
                        ₹
                      </span>
                      <input
                        type="number"
                        ...
                        max={100000}
                        onChange={(e) =>
                          updateStake(item.id, Number(e.target.value))
                        }
                        placeholder="Stake"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-1 mb-2">
                      {QUICK_STAKES.map((amount) => (
                        <button
                          key={amount}
                          className="text-xs bg-bg-card border border-stroke-light hover:bg-brand-primary hover:text-brand-text hover:border-brand-primary rounded py-1 transition-colors font-mono"
                          onClick={() =>
                            updateStake(item.id, item.stake + amount)
                          }
                        >
                    ...
                    <div className="flex justify-between text-xs">
                      <span className="text-neutral-gray-700">
                        {item.betType === "BACK" ? "Return" : "Liability"}
                      </span>
                      <span
                        className={
                          item.betType === "LAY"
          ...
          <div className="shrink-0 border-t border-stroke-light bg-bg-card p-4 space-y-2">
            ...
            <div className="space-y-1 text-xs">
              <div className="flex justify-between">
                <span className="text-neutral-gray-700">Total Stake</span>
                <span className="font-mono font-bold text-neutral-gray-800">
                  ₹{totalStake.toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-neutral-gray-700">Potential Return</span>
              ...
              <div className="flex justify-between">
                ...
                <span
                  className={clsx(
                    "font-mono font-bold",
                    totalLiability > availableBalance
                      ? "text-accent-red"
            ...
            <button
              ...
              className={clsx(
                ...
                canSubmit
                  ? "bg-cta-gradient text-brand-text hover:opacity-90"
