...
export const RunnerRow: React.FC<RunnerRowProps> = ({ runner, onBetClick, isSuspended }) => {
    // KEEP IDENTICAL: odds button renderer
    const renderOddBtn = (prices: { price: number; size: number }[], side: 'BACK' | 'LAY') => {
    ...
    return (
        <div className="flex items-center justify-between py-2.5 px-4 bg-bg-card hover:bg-bg-light-blue transition-colors">
            ...
            <div className="flex-1 min-w-0 mr-3">
                ...
                {runner.handicap !== 0 && (
                    <span className="text-[10px] text-brand-text">({runner.handicap})</span>
                )}
            </div>
