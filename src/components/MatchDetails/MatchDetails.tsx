...
const MatchDetails: React.FC = () => {
  ...
  const { matchData, scoreData, minMax, loading, error } = useMatchData({
    eventId: event_id || "",
  });
    ...
    return (
      <div className="min-h-screen bg-bg-primary flex items-center justify-center">
        <div className="bg-bg-card border border-accent-red/20 rounded-xl p-8 text-center max-w-sm mx-4">
          ...
          <p className="font-display font-semibold text-neutral-gray-800 mb-1">
            Connection Error
          </p>
  ...
  const eventName =
    matchData?.event.name || matchData?.event.eventName || "Match Details";
  const eventDate =
    matchData?.event.dateTime ||
    matchData?.event.matchDate ||
    matchData?.event.startTime;
  const isInPlay = matchData?.event.matchType === "IN_PLAY";

  return (
    <div className="min-h-screen bg-bg-primary">
      {/* Event Header */}
      <div className="bg-card-gradient text-brand-text px-4 sm:px-6 py-5 shadow-elevated">
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              {isInPlay && (
                <span className="inline-flex items-center gap-1.5 bg-accent-red text-brand-text text-xs font-bold px-2.5 py-1 rounded-full mb-2">
                  <span className="w-1.5 h-1.5 bg-white rounded-full animate-pulse" />
                  LIVE
                </span>
              )}
              <h1 className="text-xl font-display font-bold text-brand-text truncate">
                {eventName}
              </h1>
              {eventDate && (
                <div className="flex items-center gap-1.5 mt-1 text-brand-text/70 text-xs">
            ...
            {scoreData && (
              <div className="text-right bg-white/10 rounded-xl px-4 py-2 backdrop-blur-sm">
                <div
                  className="font-mono text-base font-bold text-brand-text"
            ...
            {matchData?.markets && matchData.markets.length > 0 ? (
              matchData.markets.map((marketGroup: any) => {
                ...
                return events.map((marketEvent: any) => {
                  const market: Market = {
                    marketId:
                      marketEvent.marketId || marketEvent.eid || marketEvent.id,
                    marketName:
                      marketEvent.title || marketGroup.match_data.name,
                    marketType: apiKey,
                    ...
                    runners: (marketEvent.runner || []).map((r: any) => {
                      ...
                      if (
                        ...
                        back = [
                          { price: r.backPrice1, size: r.backSize1 },
                          { price: r.backPrice2, size: r.backSize2 },
                          { price: r.backPrice3, size: r.backSize3 },
                        ];
                        lay = [
                          { price: r.layPrice1, size: r.laySize1 },
                          { price: r.layPrice2, size: r.laySize2 },
                          { price: r.layPrice3, size: r.laySize3 },
                        ];
                      } else if (
                        ["bookmaker", "bookmaker2", "toss"].includes(apiKey)
                      ) {
                        back = [
                          { price: r.back, size: r.backSize || r.line || "" },
                        ];
                        lay = [
                          { price: r.lay, size: r.laySize || r.line || "" },
                        ];
                      } else if (
                        ["session", "ballbyball", "meter", "khado"].includes(
                          apiKey,
                        )
                      ) {
                        ...
                        back = [
                          {
                            price: r.backPrice1 || r.back,
                            size: r.backSize1 || r.backSize,
                          },
                        ];
                        lay = [
                          {
                            price: r.layPrice1 || r.lay,
                            size: r.laySize1 || r.laySize,
                          },
                        ];
                      }
                      return {
              ...
              <div className="bg-bg-card rounded-xl border border-stroke-light p-10 text-center">
                <p className="text-neutral-gray-600 font-medium">
          ...
          <div className="hidden lg:flex flex-col gap-4">
            ...
            <div className="bg-neutral-gray-900 rounded-xl aspect-video flex items-center justify-center overflow-hidden relative shadow-elevated">
              ...
              <div className="relative text-center">
                <FiRadio className="w-8 h-8 text-brand-text/40 mx-auto mb-2" />
      ...
      <PlaceBetModal
        ...
        eventId={event_id || ""}
        sportName={
          matchData?.event.type ||
          matchData?.event.seriesKey?.split("_")[0] ||
          "cricket"
        }
