  ...
  const fetchBets = async () => {
    try {
      ...
      if (response.success || response.status === 200) {
        ...
        const eventBets = allBets.filter(
          (b: any) => b.eventId === eventId || b.event_id === eventId,
        );
        setBets(eventBets);
      }
    } catch (error) {
    ...
    <div className="bg-bg-card rounded-xl border border-stroke-light shadow-betting-card overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-bg-light-blue border-b border-stroke-light">
        <div className="flex items-center gap-2">
          ...
          <span className="font-display font-semibold text-sm text-neutral-gray-800">
            My Bets
          </span>
        </div>
        <span className="bg-brand-primary text-brand-text text-xs font-bold px-2 py-0.5 rounded-full">
          {bets.length}
        </span>
      </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-stroke-light bg-bg-light-blue/50">
              <th className="py-2 px-3 text-left font-semibold text-neutral-gray-700 uppercase tracking-wider">
                Runner
              </th>
              <th className="py-2 px-3 text-center font-semibold text-neutral-gray-700 uppercase tracking-wider">
                Odds
              </th>
              <th className="py-2 px-3 text-center font-semibold text-neutral-gray-700 uppercase tracking-wider">
                Stake
              </th>
              <th className="py-2 px-3 text-right font-semibold text-neutral-gray-700 uppercase tracking-wider">
                P/L
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-stroke-light">
            {bets.map((bet, index) => (
              ...
              >
                <td
                  className={`py-2.5 px-3 border-l-2 ${bet.side === "BACK" ? "border-odds-back" : "border-odds-lay"}`}
                >
                  <p className="font-medium text-neutral-gray-800 truncate max-w-[100px]">
                    {bet.runnerName}
                  </p>
                  <p className="text-[10px] text-brand-text truncate max-w-[100px]">
                    {bet.marketName}
                  </p>
                </td>
                <td className="py-2.5 px-3 text-center font-mono text-brand-text">
                  {bet.odds}
                </td>
                <td className="py-2.5 px-3 text-center font-mono text-brand-text">
                  ₹{bet.stake}
                ...
                <td
                  className={`py-2.5 px-3 text-right font-mono font-bold ${bet.side === "BACK" ? "text-accent-green" : "text-accent-red"}`}
                >
                  {bet.side === "BACK"
