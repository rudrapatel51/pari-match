...
const AffiliateCommissions: React.FC = () => {
    ...
    useEffect(() => { load(page, filters); }, [page, load, filters]);
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="space-y-2">
                <p className="text-xs font-semibold text-neutral-gray-700 uppercase tracking-wider">Status</p>
                ...
                {/* Type filter pills */}
                <p className="text-xs font-semibold text-neutral-gray-700 uppercase tracking-wider pt-1">Commission Type</p>
                <div className="flex flex-wrap gap-2">
                    {TYPE_FILTERS.map(t => {
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-stroke-light bg-bg-light-blue">
                                    {['Player', 'Type', 'Trigger Amount', 'Rate', 'Commission', 'Status', 'Date'].map(h => (
                            ...
                            <tbody className="divide-y divide-stroke-light">
                                {commissions.map(c => (
                                    <tr key={c._id} className="hover:bg-bg-light-blue transition-colors">
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={clsx(
                                                ...
                                                TYPE_BADGE[c.commission_type] || 'bg-bg-light-blue text-brand-text'
                                            )}>
                                                {c.commission_type}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={clsx(
                                                'px-2 py-0.5 rounded-full text-xs font-semibold capitalize',
                                                STATUS_BADGE[c.status] || 'bg-bg-light-blue text-brand-text'
                                            )}>
                                                {c.status || 'pending'}
                                            </span>
                                        </td>
