...
import {
    FiUsers, FiDollarSign, FiActivity, FiFileText, FiTrendingUp,
    FiCopy, FiLink, FiShare2, FiCheckCircle, FiShield,
} from 'react-icons/fi';
import { AffiliateStats, AffiliateApplicationStatus } from '../../types/affiliate';
...
const StatCard: React.FC<{
    ...
    <div className="bg-bg-card border border-stroke-light rounded-xl shadow-betting-card p-4 flex items-center gap-3">
        <div className={clsx('w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 bg-brand-primary/10', accentClass)}>
            <Icon className="w-5 h-5" />
        </div>
        <div className="min-w-0">
            <p className="text-xs text-neutral-gray-700 truncate">{label}</p>
            <p className="text-base font-mono font-bold text-brand-text mt-0.5 truncate">{value}</p>
        </div>
    </div>
);
...
const AffiliateDashboard: React.FC = () => {
    ...
    // ── State ─────────────────────────────────────────────────────────────────
    const [appStatus, setAppStatus] = useState<AffiliateApplicationStatus | null>(null);
        ...
        return (
            <div className="min-h-screen bg-bg-primary">
                <div className="bg-brand-primary px-6 py-5">
                    <h1 className="text-xl font-display font-bold text-white tracking-wide">Affiliate Program</h1>
                    <p className="text-sm text-white/60 mt-0.5">Earn commissions by referring players to the platform</p>
                </div>
    ...
    if (status === 'pending' || status === 'rejected' || status === 'blocked' || status === 'deleted') {
        return (
            <div className="min-h-screen bg-bg-primary">
                <div className="bg-brand-primary px-6 py-5">
                    <h1 className="text-xl font-display font-bold text-white tracking-wide">Affiliate Program</h1>
        ...
        <div className="min-h-screen bg-bg-primary">
            ...
            <div className="bg-brand-primary px-6 py-5">
                <div className="flex items-start justify-between gap-3">
                    ...
                    {affiliateType && (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-white/15 border border-white/20 rounded-full text-xs font-semibold text-white capitalize flex-shrink-0 mt-1">
                    ...
                    <div className="bg-bg-card border border-stroke-light rounded-xl shadow-betting-card p-4 space-y-3">
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <button
                                ...
                                className="inline-flex items-center gap-1.5 px-4 py-2 bg-brand-primary text-white text-sm font-semibold rounded-xl hover:opacity-90 transition-opacity flex-shrink-0"
                            >
                        ...
                        {referralLink && (
                            <>
                                <div className="flex items-center gap-2 bg-bg-secondary rounded-lg px-3 py-2 border border-stroke-light">
                                    ...
                                    <p className="text-xs text-brand-text truncate flex-1 font-mono">{referralLink}</p>
                                    <button
                                ...
                                <div className="flex items-center gap-2 pt-1 flex-wrap">
                                    ...
                                    <a
                                        ...
                                        className="inline-flex items-center gap-1 px-3 py-1 bg-[#25D366] text-white text-xs font-semibold rounded-lg hover:opacity-90 transition-opacity"
                                    >
                                        WhatsApp
                                    </a>
                                    <a
                                        ...
                                        target="_blank" rel="noopener noreferrer"
                                        className="inline-flex items-center gap-1 px-3 py-1 bg-[#0088CC] text-white text-xs font-semibold rounded-lg hover:opacity-90 transition-opacity"
                                    >
                                        Telegram
                                    </a>
                                </div>
                            ...
                            <div className="space-y-4">
                                ...
                                {stats?.last_updated && (
                                    <p className="text-xs text-neutral-gray-600">
                                        Stats updated:{' '}
                                        <span className="font-mono">{new Date(stats.last_updated).toLocaleString('en-IN')}</span>
                                    </p>
                                )}
                            </div>
                        )}
