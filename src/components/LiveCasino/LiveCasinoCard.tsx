...
const LiveCasinoCard: React.FC<LiveCasinoCardProps> = ({ game, onPlay }) => {
    ...
    const handlePlay = async () => {
        ...
        try {
            ...
            const url = res?.data?.data?.url || res?.data?.url || res?.data?.launch_url;
    ...
    return (
        <div className="group relative rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer" onClick={handlePlay}>
            ...
            <div className="relative aspect-[3/2] overflow-hidden">
                ...
                <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center z-20">
                    <button className="bg-bg-card hover:bg-bg-light-blue text-brand-text rounded-full p-4 transform scale-0 group-hover:scale-100 transition-all duration-300 shadow-lg" disabled={launching}>
                        {launching ? <span className="w-8 h-8 block border-2 border-brand-text border-t-transparent rounded-full animate-spin" /> : <FiPlay className="w-8 h-8" />}
                    </button>
                </div>
                ...
                {game.isLive && (
                    <div className="absolute top-2 right-2 z-10">
                        <div className="flex items-center gap-1 bg-accent-red text-white text-[10px] font-bold px-2 py-1 rounded">
