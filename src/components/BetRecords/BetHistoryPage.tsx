...
const BetTypePill: React.FC<{ type: string }> = ({ type }) => (
  <span
    className={clsx(
      "inline-flex items-center px-2 py-0.5 text-[10px] font-bold uppercase",
      type === "BACK"
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  const styles: Record<string, string> = {
    WON: "bg-accent-green text-brand-text",
    LOST: "bg-accent-red text-brand-text",
    VOID: "bg-brand-text/40 text-brand-text",
    PENDING: "bg-accent-yellow text-brand-text",
  };
  return (
    <span
      className={clsx(
...
const BetHistoryPage: React.FC = () => {
  const { betHistory, setBetHistory, totalPages, isLoading, setLoading } =
    useBetStore();
  ...
  const loadBets = async (p = page) => {
    setLoading(true);
    try {
      const res = (await userApi.getBetHistory({
        page: p,
        from_date: fromDate,
        to_date: toDate,
      })) as any;
  ...
  return (
    <div className="min-h-screen bg-bg-primary">
      ...
      <div className="p-4 md:p-6 space-y-4">
        {/* ── Filter card ── */}
        <div className="bg-bg-card border border-stroke-light shadow-betting-card p-6 md:p-8">
          <div className="flex items-center gap-2 mb-4">
            {/*
                        ...
                        */}
            <div className="w-8 h-8 bg-brand-primary-dark flex items-center justify-center">
        ...
        {/* ── Table card ── */}
        <div className="bg-bg-card border border-stroke-light shadow-betting-card">
          {isLoading ? (
                  ...
                  <thead>
                    <tr className="bg-brand-primary-dark border-b border-stroke-light">
                      {[
                        ...
                        <th
                          key={h}
                          className="text-left py-3 px-3 text-[10px] font-bold text-brand-text/70 uppercase tracking-wider whitespace-nowrap"
                        >
                          {h}
                        </th>
                      ))}
