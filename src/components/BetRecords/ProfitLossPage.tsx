...
const ProfitLossPage: React.FC = () => {
  ...
  const loadData = useCallback(
    async (from: string, to: string) => {
      setLoading(true);
      try {
        const res = (await userApi.getProfitLoss({
          from_date: from,
          to_date: to,
        })) as any;
  ...
  const totalPL = profitLoss.reduce(
    (sum: number, r: any) => sum + (parseFloat(r.profit_loss) || 0),
    0,
  );
  const maxAbsPL =
    profitLoss.length > 0
      ? Math.max(
          ...profitLoss.map((r: any) =>
            Math.abs(parseFloat(r.profit_loss) || 0),
          ),
        )
      : 1;
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="bg-bg-card border border-stroke-light rounded-xl shadow-betting-card p-4 md:p-5 mb-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-display font-semibold text-base text-neutral-gray-800">
              Date Range
            </h2>
          </div>
          <DateRangeFilter
        ...
        {profitLoss.length > 0 && (
          ...
          >
            ...
            <div>
              <p className="text-xs font-semibold text-brand-text uppercase tracking-wider mb-0.5">
                Total P&amp;L
              </p>
              <p
                className={clsx(
              ...
              <table className="w-full text-sm min-w-[520px]">
                <thead>
                  <tr className="bg-bg-light-blue border-b border-stroke-light">
                    {["Event / Market", "Bets", "Stake", "P&L", "Date"].map(
                      (h) => (
                        <th
                          key={h}
                          className="text-left py-2.5 px-3 text-xs font-semibold text-brand-text uppercase tracking-wider"
                        >
                          {h}
                        </th>
                      ),
                ...
                <tbody className="divide-y divide-stroke-light/50">
                  {profitLoss.map((r: any, i: number) => {
                    ...
                    const barWidth =
                      maxAbsPL > 0
                        ? Math.round((Math.abs(pl) / maxAbsPL) * 100)
                        : 0;
                    return (
                      <tr
                        key={i}
                      ...
                      >
                        ...
                        <td className="py-2.5 px-3 text-brand-text text-xs">
                          {r.bets_count || "—"}
                        </td>
