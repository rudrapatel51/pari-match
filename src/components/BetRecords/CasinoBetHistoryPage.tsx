...
const CasinoBetHistoryPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    casinoApi
      ...
      .catch((e: any) =>
        toast.error(e?.message || "Failed to load casino bets"),
      )
      ...
      <div className="p-4 md:p-6">
        <div className="bg-bg-card border border-stroke-light rounded-xl shadow-betting-card overflow-hidden">
          ...
          {loading ? (
            ...
            <EmptyState
              title="No Casino Bets"
              description="You haven't placed any casino bets yet."
            />
          ) : (
            <>
                ...
                <table className="w-full text-sm min-w-[560px]">
                  <thead>
                    <tr className="border-b border-stroke-light bg-bg-light-blue/50">
                      {[
                        ...
                        <th
                          key={h}
                          className="text-left py-2.5 px-4 text-xs font-semibold text-brand-text uppercase tracking-wider"
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody className="divide-y divide-stroke-light/50">
                    {bets.map((bet: any, i: number) => {
                      ...
                      const gameName =
                        bet.game_name || bet.gameName || "Casino";
                        ...
                        >
                          ...
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-2.5">
                              ...
                              <span className="font-medium text-brand-text text-xs truncate max-w-[100px]">
                                {gameName}
                              </span>
                            </div>
                          </td>
                          ...
                          <td
                            className={`py-3 px-4 font-mono font-semibold text-xs ${isWin ? "text-accent-green" : "text-neutral-gray-600"}`}
                          >
                            ₹{bet.win_amount || bet.winAmount || 0}
                          </td>
                          <td className="py-3 px-4">
                            <span
                              className={`inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full ${isWin ? "bg-accent-green/10 text-accent-green" : "bg-accent-red/10 text-accent-red"}`}
                            >
                              {isWin ? (
                                <>
