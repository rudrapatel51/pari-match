...
const UnsettledBetsPage: React.FC = () => {
  ...
  const { unsettledBets, setUnsettledBets, isLoading, setLoading } =
    useBetStore();
    ...
    <div className="min-h-screen bg-bg-primary">
      ...
      <div className="bg-brand-primary px-6 py-5">
        ...
        <p className="text-sm text-brand-text/60 mt-0.5">
          Currently open bets awaiting settlement
        </p>
      </div>
        ...
        <div className="bg-bg-card border border-stroke-light rounded-xl shadow-betting-card overflow-hidden">
          ...
          {isLoading ? (
            ...
            <EmptyState
              ...
              description="All your bets have been settled."
            />
          ) : (
                ...
                <tbody>
                  {unsettledBets.map((bet: any) => (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3 font-medium text-neutral-gray-800 text-xs">
                        {bet.runner_name || "-"}
                      </td>
                      <td className="py-2.5 px-3">
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-bold ${bet.side === "BACK" ? "bg-odds-back/20 text-odds-back" : "bg-odds-lay/20 text-odds-lay"}`}
                        >
                          {bet.side}
                        </span>
                      </td>
                      <td className="py-2.5 px-3 font-mono text-brand-text text-xs">
                        {bet.odds}
                      </td>
                      <td className="py-2.5 px-3 font-mono text-brand-text text-xs">
                        ₹{bet.stake}
