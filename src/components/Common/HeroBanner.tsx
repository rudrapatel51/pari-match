...
const HeroBanner: React.FC<HeroBannerProps> = ({
  ...
  if (loading) {
    return (
      <div
        className={`${sizeClass} bg-brand-primary-dark animate-pulse  w-full`}
      />
    );
  }
  ...
  if (banners.length === 0) {
    return (
      <div
        className={`relative ${sizeClass} flex items-center justify-center overflow-hidden bg-gradient-to-r from-brand-secondary to-brand-primary  w-full`}
      ></div>
    );
  }
    ...
    <div className={`relative ${sizeClass} overflow-hidden select-none w-full`}>
      ...
      {banners.length > 1 && (
        <>
          <button
            onClick={prev}
            className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/60 text-brand-text rounded-full p-1.5 transition-colors"
            aria-label="Previous"
          >
          ...
          <button
            onClick={next}
            className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/60 text-brand-text rounded-full p-1.5 transition-colors"
            aria-label="Next"
          >
